...
const PropertyDetails = () => {
  ...
  const gallery = property ? (property.gallery.length ? property.gallery : [{ url: property.image, alt: property.title }]) : [];
      ...
      <main className="pt-32 pb-24">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10 text-sm text-muted-foreground">
            ...
            <span>
              ID #{property.id.toString().padStart(3, "0")} • {property.type} • {property.area}m²
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2">
              <div className="relative rounded-3xl mb-8 shadow-2xl bg-card">
                <Badge className="absolute z-20 top-6 left-6 bg-property-badge text-white text-base px-4 py-2 uppercase tracking-[0.15em]">
                  {property.transaction}
                </Badge>
                <Badge className="absolute z-20 top-6 right-6 bg-card text-card-foreground text-base px-4 py-2 uppercase tracking-[0.15em]">
                  {property.type}
                </Badge>
              ...
              <div className="bg-card rounded-3xl p-8 border border-border/60 shadow-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {property.amenities.map((amenity, index) => {
                    ...
                    const hasSecurity = normalized.includes("segurança") || normalized.includes("concierge");
